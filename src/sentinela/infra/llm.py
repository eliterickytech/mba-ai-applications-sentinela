...
- Produza de 3 a 5 bullets e de 1 a 3 objetos_destaque.
...
def gerar_resumo(df: pd.DataFrame) -> RelatorioSemanal:
    """Chama o GPT com structured output e devolve um RelatorioSemanal validado."""
    from openai import OpenAI  # import tardio: só é preciso no caminho com IA
    ...
    # response_format = modelo Pydantic: a OpenAI força a saída a bater com o schema.
    completacao = cliente.beta.chat.completions.parse(
        model=MODELO,
        temperature=0,
        messages=[
        ...
        response_format=RelatorioSemanal,
    )

    relatorio = completacao.choices[0].message.parsed
    if relatorio is None:
        raise RuntimeError("O modelo não retornou a saída estruturada esperada.")
    return relatorio
